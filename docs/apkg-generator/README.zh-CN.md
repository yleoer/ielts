...
- `docs/apkg-generator/anki-card-template.md` - 卡片设计方案和模板参考。
- `apkg-generator/scripts/generate_anki_import.py` - 生成 TSV 和 APKG 的脚本。
- `apkg-generator/data/vocabulary.txt` - 词汇源文件，来自
  [`hefengxian/my-ielts`](https://github.com/hefengxian/my-ielts) 项目。
- `apkg-generator/data/audio/` - 本地单词发音音频。
- `apkg-generator/anki_export/cache/` - 音标和 AI 生成内容缓存。
...
词汇源文件 `apkg-generator/data/vocabulary.txt` 来自
[`hefengxian/my-ielts`](https://github.com/hefengxian/my-ielts) 项目。
...
1. 打开 Anki。
2. 选择 `文件 -> 导入`。
3. 选择 `apkg-generator/anki_export/ielts_vocabulary.apkg`。
4. 导入牌组。
